...
## PR
大学在学中にプログラミングに興味を持つようになり、休学してKotlinでのAndroidアプリを制作しました。  
...
直近では、Flutterによるクロスプラットフォーム開発に興味を持ち、Flutterを用いた画像付き投稿SNSアプリを個人開発しました。

Flutterの経験は個人開発のみですが、Androidアプリ開発の経験(2年弱)があるのでネイティブアプリ開発の知見を活かせることが強みです。詳細設計からテスト・運用保守の工程を経験し、既存機能の改修および新規機能開発の経験があるため、一般的なアプリ開発の業務フローは心得ております。

未経験の技術であっても、キャッチアップしつつ業務を進めることが可能です。例として、未経験のFlutterとFirebaseを独学して個人制作しました。また、Androidのビジネスチャットアプリの開発で、UIテストの自動化について自分含めメンバー全員が未経験であったため、自身で公式ドキュメント等を調べながら実装しました。

## スキルセット
### 言語
Dart(初級) | Android Java(上級) | Java(中級)

### フレームワーク・ライブラリ
Flutter(初級) | Spring Boot(中級)

### RDB/NoSQL
SQLite | Realm | Oracle DB | Firestore

### クラウド
Firebase(Firestore, Storage, Authentication)

## 主な業務経歴
### 画像付きポスト投稿SNSアプリ【個人開発】(2022/09 - 2022/10)
ログイン制の画像付きポスト(記事)を投稿できるSNSアプリの個人開発。  
※Google Playストアに申請中。

[GitHubのリポジトリ]  
https://github.com/yutohino/esu-n-esu

【使用した技術・ツール】  
Dart(Flutter) | Android Stuido | Provider | Firebase(Firestore, Storage, Authentication) | GitHub | Figma | Trello
...
#### 学んだこと
- Dart・Flutterの概念および基礎と、Flutter・Firebaseを用いた基本的なスマホアプリ開発の方法を学習した。FlutterはAndroid開発の経験によりスムーズに理解できた。

#### 苦労したこと
- iOS開発の経験がないため、iOS端末とシミュレータで動作するのに必要なセットアップ等に躓いた。この経験を通じてiOSのハードウェア機能にアクセスする方法やデザインと挙動の違い等の知見を得た。
